"use client";
import { useState } from "react";

export default function BudgetChoice({ onNext }) {
  const [selectedBudget, setSelectedBudget] = useState(null);

  const budgets = [
    { id: 1, label: "Less than $100", value: "under100" },
    { id: 2, label: "More than $100", value: "over100" },
  ];

  const handleSelect = (value) => {
    setSelectedBudget((prev) => (prev === value ? null : value));
  };

  const handleContinue = () => {
    if (selectedBudget) {
      onNext?.(selectedBudget);
    } else {
      alert("Please choose your budget before continuing.");
    }
  };

  return (
    <div className="p-6 flex flex-col items-center">
      <h2 className="text-xl font-semibold mb-6 text-center">
        Choose your budget
      </h2>

      <div className="flex flex-col sm:flex-row gap-6">
        {budgets.map((item) => {
          const isSelected = selectedBudget === item.value;
          return (
            <div
              key={item.id}
              onClick={() => handleSelect(item.value)}
              className={`cursor-pointer px-8 py-6 rounded-2xl shadow-md text-center text-lg font-medium transition-all duration-300 ${
                isSelected
                  ? "bg-blue-600 text-white scale-105"
                  : "bg-gray-100 hover:bg-gray-200"
              }`}
            >
              {item.label}
            </div>
          );
        })}
      </div>

      <button
        className="mt-8 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition"
        onClick={handleContinue}
      >
        Continue
      </button>
    </div>
  );
}
